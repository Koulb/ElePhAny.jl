...
parser = argparse.ArgumentParser(description='Process input paths and parameters.')
parser.add_argument('--path_to_kcw', type=str, default='./displacements/scf_0/tmp/scf.save/', help='Path to kcw directory')
parser.add_argument('--path_to_save', type=str, default='./si_dft.save/', help='Path to save directory')
parser.add_argument('--path_to_out', type=str, default='./si.save/', help='Path to output directory')
parser.add_argument('--nks', type=int, default=2**3, help='Number of k-points')
parser.add_argument('--type_of_calc', type=str, default='dft', choices=['dft', 'kcw'], help='Type of calculation')

args = parser.parse_args()

path_to_kcw = args.path_to_kcw
path_to_save = args.path_to_save
path_to_out  = args.path_to_out
nks = args.nks
type_of_calc = args.type_of_calc
...
for index_k, ks_energies in enumerate(tree_qe.iter('ks_energies')):
    ...
    if type_of_calc == 'kcw':
        for index_e, energy_qe in enumerate(energies_qe):
            energies_qe_raw = energies_qe_raw.replace(energy_qe,energies_kp[index_e],1) 
    else:
...
if type_of_calc == 'kcw':
    for i in range(nks):
        ...
        #src_file = path_to_kcw + f'wfcdw{i+1}.dat'

        dest_file = path_to_out + f'wfc{i+1}.dat'
        shutil.copy(src_file, dest_file)

    shutil.copy(path_to_save + 'charge-density.dat', path_to_out)
    pass
else:
    for i in range(nks):
        src_file = path_to_kcw + f'wfc{i+1}.dat'
        dest_file = path_to_out + f'wfc{i+1}.dat'
        shutil.copy(src_file, dest_file)
